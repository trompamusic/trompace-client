...
from trompace.application.application import create_entrypointcontrolaction_CE, create_property_CE, create_propertyvalue_CE, create_application_CE, subscribe_controlaction
...
async def main(app_config_file, ep_config_file):
    await create_entrypoint(app_config_file, ep_config_file)        # Make sure the application and the entry point exist, if not, create
...
async def create_entrypoint(app_config_file='./docs/app_config.ini', ep_config_file='./docs/ep_config.ini'):
    ...
    assert all(x in app.keys() for x in check_app), "{} not present in application config file".format([x for x in check_app if x not in app.keys()]).replace('[','').replace(']','')
    ...
    assert all(x in config_ep.keys() for x in check_eps), "{} not present in entry point config file".format([x for x in check_eps if x not in config_ep.keys()]).replace('[','').replace(']','')
    ...
    assert all(x in ep.keys() for x in check_ep), "{} not present in entry point config file".format([x for x in check_ep if x not in ep.keys()]).replace('[','').replace(']','')
    ...
    assert all(x in ca.keys() for x in check_ca), "{} not present in entry point config file".format([x for x in check_ca if x not in ca.keys()]).replace('[','').replace(']','')
    ...
    for i in range(num_properties):
        ...
        assert all(x in pro.keys() for x in check_pro), "{} not present in entry point config file for property {}".format([x for x in check_pro if x not in pro.keys()], i+1).replace('[','').replace(']','')
    ...
    for i in range(num_propertyvalues):
        ...
        check_pro = ['name', 'description', 'defaultValue',  'valuemaxlength','valueminlength', 'multiplevalues', 'valuename', 'valuepattern', 'valuerequired']
        assert all(x in pro.keys() for x in check_pro), "{} not present in entry point config file for property value specification {}".format([x for x in check_pro if x not in pro.keys()], i+1).replace('[','').replace(']','')
        value_name = pro['name']
